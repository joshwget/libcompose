...
func ConvertV1toV2(v1Services map[string]*ServiceConfigV1, environmentLookup EnvironmentLookup, resourceLookup ResourceLookup) (map[string]*ServiceConfig, map[string]*VolumeConfig, map[string]*NetworkConfig, error) {
	...
	for name, _ := range v2Services {
		v2Services[name].Build = builds[name]
	}

	return v2Services, nil, nil, nil
}
