...
var constraintSchemaLoader gojsonschema.JSONLoader
var fieldsSchema map[string]interface{}

type environmentFormatChecker struct{}
...
func (f portsFormatChecker) IsFormat(input string) bool {
	_, _, err := nat.ParsePortSpecs([]string{input})
	return err == nil
}

func (f exposeFormatChecker) IsFormat(input string) bool {
	_, _, err := nat.ParsePortSpecs([]string{input})
	return err == nil
}
...
func setupSchemaLoaders() error {
	if fieldsSchema == nil {
		...
		fieldsSchema = schemaRaw.(map[string]interface{})
	}
	if schemaLoader == nil {
		gojsonschema.FormatCheckers.Add("environment", environmentFormatChecker{})
		gojsonschema.FormatCheckers.Add("ports", portsFormatChecker{})
		gojsonschema.FormatCheckers.Add("expose", exposeFormatChecker{})
		schemaLoader = gojsonschema.NewStringLoader(schemaString)
	}
	if constraintSchemaLoader == nil {
		definitions := fieldsSchema["definitions"].(map[string]interface{})
