...
func getValue(serviceMap RawServiceMap, context string) string {
...
func oneOfMessage(serviceMap RawServiceMap, schema map[string]interface{}, err, nextErr gojsonschema.ResultError) string {
	...
	case "invalid_type":
		...
		validTypes := parseValidTypesFromSchema(fieldsSchema, err.Context().String())
	...
	if !result.Valid() {
		for i := 0; i < len(result.Errors()); i++ {
			...
			if err.Context().String() == "(root)" {
				switch err.Type() {
				case "additional_property_not_allowed":
					finalErrors = append(finalErrors, fmt.Sprintf("Invalid service name '%s' - only [a-zA-Z0-9\\._\\-] characters are allowed", err.Field()))
				default:
					finalErrors = append(finalErrors, err.Description())
				}
			} else {
				...
				switch err.Type() {
				case "additional_property_not_allowed":
					finalErrors = append(finalErrors, unsupportedConfigMessage(key, result.Errors()[i+1]))
				case "number_one_of":
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' configuration key '%s' %s", serviceName, key, oneOfMessage(serviceMap, fieldsSchema, err, result.Errors()[i+1])))

					// Next error handled in oneOfMessage, skip over it
					i++
				case "invalid_type":
					finalErrors = append(finalErrors, invalidTypeMessage(serviceName, key, err))
				case "required":
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' option '%s' is invalid, %s", serviceName, key, err.Description()))
				case "missing_dependency":
					dependency := err.Details()["dependency"].(string)
					finalErrors = append(finalErrors, fmt.Sprintf("Invalid configuration for '%s' service: dependency '%s' is not satisfied", serviceName, dependency))
				case "unique":
					contextWithDuplicates := getValue(serviceMap, err.Context().String())
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' configuration key '%s' value %s has non-unique elements", serviceName, key, contextWithDuplicates))
				default:
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' configuration key %s value %s", serviceName, key, err.Description()))
				}
			}
		}

		return fmt.Errorf(strings.Join(finalErrors, "\n"))
	}
		...
		for _, err := range result.Errors() {
			if err.Type() == "number_any_of" {
				...
				if containsImage && containsBuild {
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' has both an image and build path specified. A service can either be built to image or use an existing image, not both.", serviceName))
				} else if !containsImage && !containsBuild {
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' has neither an image nor a build path specified. Exactly one must be provided.", serviceName))
				} else if containsImage && containsDockerfile {
					finalErrors = append(finalErrors, fmt.Sprintf("Service '%s' has both an image and alternate Dockerfile. A service can either be built to image or use an existing image, not both.", serviceName))
				}
			}
		}
