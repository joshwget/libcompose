...
func NewVolume(name string, config *config.VolumeConfig, client client.APIClient) *Volume {
	return &Volume{
		client:        client,
		name:          name,
...
func VolumesFromServices(cli client.APIClient, volumeConfigs map[string]*config.VolumeConfig, services *config.ServiceConfigs, volumeEnabled bool) (*Volumes, error) {
	var err error
	volumes := make([]*Volume, 0, len(volumeConfigs))
	for name, config := range volumeConfigs {
		volume := NewVolume(name, config, cli)
...
func (f *DockerFactory) Create(volumeConfigs map[string]*config.VolumeConfig, serviceConfigs *config.ServiceConfigs, volumeEnabled bool) (project.Volumes, error) {
	cli := f.ClientFactory.Create(nil)
	return VolumesFromServices(cli, volumeConfigs, serviceConfigs, volumeEnabled)
}
